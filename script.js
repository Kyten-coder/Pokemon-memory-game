...
function startGame(rows, cols) {
  ...
  const selectedImages = shuffle(pokemonImages).slice(0, totalCards / 2);
  cardValues = shuffle([...selectedImages, ...selectedImages]);
...
function flipCard(cardElement) {
  if (flippedCards.length === 2 || matchedCards.includes(cardElement)) return;
...
function checkMatch() {
  ...
  if (card1.dataset.cardValue === card2.dataset.cardValue) {
    ...
    if (matchedCards.length === gridRows * gridCols) {
      endGame();
    }
  } else {
    setTimeout(() => {
